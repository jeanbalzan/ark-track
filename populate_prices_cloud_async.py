...
async def get_stocks():
    ...
    pool = await asyncpg.create_pool(config_polygon.DB_URL, user=config_polygon.DB_USER, password=config_polygon.DB_PASS,min_size=7,max_size=24, command_timeout=60)

    # get a connection
    async with pool.acquire() as connection:
        stocks = await connection.fetch("SELECT * FROM stock WHERE symbol IN ('AAPL','TSLA','MSFT','META','GOOG','NFLX')")

        #### Need to add daterange loop and also max size loop

        symbol_urls = {}
        for stock in stocks:
            symbol_urls[stock['id']] = f"https://api.polygon.io/v2/aggs/ticker/{stock['symbol']}/range/1/minute/2022-06-01/2022-06-30?apiKey={config_polygon.API_KEY}&limit=50000"
